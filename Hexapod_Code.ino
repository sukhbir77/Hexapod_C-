...
void loop() {
  ...
  double joy1x = map(127, 0, 254, -100, 100);
  double joy1y = map(254, 0, 254, -100, 100);

  // Serial.println((String)"The value of x is: " + joy1x);
  // Serial.println((String)"The value of y is: " + joy1y);

  double joy2x = map(127, 0, 254, -100, 100);
  double joy2y = map(127, 0, 254, -100, 100);
  ...
  distanceFromGround = distanceFromGroundBase + 25 * -1.7;
...
void moveToPos(int leg, Vector3 pos) {
  ...
  switch (leg) {
    case 0:
      myServo.setPWM(15, 0, map(targetRot.x, 0, 180, 125, 600));
      backFemur.writeMicroseconds(femurMicroseconds);
      backTibia.writeMicroseconds(tibiaMicroseconds);
      break;

    case 1:
      myServo.setPWM(12, 0, map(targetRot.x, 0, 180, 125, 600));
      myServo.setPWM(13, 0, map(targetRot.y, 0, 180, 125, 600));
      myServo.setPWM(14, 0, map(targetRot.z, 0, 180, 125, 600));
      break;

    case 2:
      myServo.setPWM(9, 0, map(targetRot.x, 0, 180, 125, 600));
      myServo.setPWM(10, 0, map(targetRot.y, 0, 180, 125, 600));
      myServo.setPWM(11, 0, map(targetRot.z, 0, 180, 125, 600));
      break;

    case 3:
      myServo.setPWM(0, 0, map(targetRot.x, 0, 180, 125, 600));
      myServo.setPWM(1, 0, map(targetRot.y, 0, 180, 125, 600));
      myServo.setPWM(2, 0, map(targetRot.z, 0, 180, 125, 600));
      break;

    case 4:
      myServo.setPWM(3, 0, map(targetRot.x, 0, 180, 125, 600));
      myServo.setPWM(4, 0, map(targetRot.y, 0, 180, 125, 600));
      myServo.setPWM(5, 0, map(targetRot.z, 0, 180, 125, 600));
      break;

    case 5:
      myServo.setPWM(6, 0, map(targetRot.x, 0, 180, 125, 600));
      myServo.setPWM(7, 0, map(targetRot.y, 0, 180, 125, 600));
      myServo.setPWM(8, 0, map(targetRot.z, 0, 180, 125, 600));
      break;
